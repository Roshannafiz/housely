...
var DateTimePicker = function ($, moment) {
  ...
  var trim = function trim(str) {
    return str.replace(/(^\s+)|(\s+$)/g, '');
      ...
      Selector = {
    DATA_TOGGLE: "[data-toogle=\"" + DATA_KEY + "\"]"
  },
      ClassName = {
    INPUT: NAME + "-input"
  },
...
var TempusDominusBootstrap4 = function ($) {
  ...
  var TempusDominusBootstrap4 = /*#__PURE__*/function (_DateTimePicker) {
    ...
    _proto2._init = function _init() {
      if (this._element.hasClass('input-group')) {
        ...
        if (datepickerButton.length === 0) {
          this.component = this._element.find('[data-toogle="datetimepicker"]');
        } else {
    ...
    _proto2._getTemplate = function _getTemplate() {
      var template = $('<div>').addClass(("bootstrap-datetimepicker-widget dropdown-menu " + (this._options.calendarWeeks ? 'tempusdominus-bootstrap-datetimepicker-widget-with-calendar-weeks' : '') + " " + ((this._useFeatherIcons() ? 'tempusdominus-bootstrap-datetimepicker-widget-with-feather-icons' : '') + " ")).trim()),
          ...
          toolbar = $('<li>').addClass(("picker-switch" + (this._options.collapse ? ' accordion-toogle' : '') + " " + ("" + (this._useFeatherIcons() ? 'picker-switch-with-feathers-icons' : ''))).trim()).append(this._getToolbar());
        ...
        case 'togglePicker':
          {
            ...
            if (expanded && expanded.length) {
              ...
              if (expanded.collapse) {
                ...
                // otherwise just toogle in class on the two views
  ...
  */
  ...
  $(document).on(DateTimePicker.Event.CLICK_DATA_API, DateTimePicker.Selector.DATA_TOGGLE, function () {
    ...
    if (config._options.allowInputToggle && $originalTarget.is('input[data-toogle="datetimepicker"]')) {
      return;
    }
